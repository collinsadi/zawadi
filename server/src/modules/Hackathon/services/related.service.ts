...
export async function getRelatedHackathons(userAddress: string): Promise<RelatedHackathon[]> {
  const address = normalizeAddress(userAddress);
  if (!address) return [];

  // Fetch organiser hackathons directly
  const organiserHacks = await Hackathon.find({ organiserAddress: new RegExp(`^${address}$`, 'i') }).lean();
  const organiserMap = new Map<string, RelatedHackathon>();
  organiserHacks.forEach((h) => {
    organiserMap.set(String(h._id), { ...(h as any), relationships: ["organiser"] });
  });

  // Fetch sponsor challenges -> hackathons
  const sponsorChallenges = await Challenge.find({ sponsorAddress: new RegExp(`^${address}$`, 'i') }, { hackathon: 1 }).lean();
  const sponsorHackIds = new Set<string>(sponsorChallenges.map((c) => String(c.hackathon)));

  // Fetch winner allocations -> challenges -> hackathons
  const winnerAllocations = await Allocation.find({ winnerAddress: new RegExp(`^${address}$`, 'i') }, { challenge: 1 })
    .populate({ path: 'challenge', select: 'hackathon', options: { lean: true } as any })
    .lean();
  ...
  const missingHackIds = Array.from(allHackIds).filter((id) => !organiserMap.has(id));
  let missingDocs: IHackathon[] = [];
  if (missingHackIds.length) {
    missingDocs = await Hackathon.find({ _id: { $in: missingHackIds } }).lean();
  }
  ...
  sponsorHackIds.forEach((id) => {
    ...
    if (entry) {
      if (!entry.relationships.includes("sponsor")) entry.relationships.push("sponsor");
    }
  });
  ...
  winnerHackIds.forEach((id) => {
    const entry = resultMap.get(id);
    if (entry) {
      if (!entry.relationships.includes("winner")) entry.relationships.push("winner");
    }
  });
